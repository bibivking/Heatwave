...
def plot_time_series(var,var_name,case_name, units= None, val_min=None,val_max=None,lvl=None,latidxs=None,lonidxs=None):
    ...
    if lvl == None:
        if latidxs==None or lonidxs==None:
            fig.savefig("./plots/time_series_spinup_"+var_name+"_"+case_name_string , bbox_inches='tight', pad_inches=0.1)
        else:
            fig.savefig("./plots/time_series_spinup_"+var_name+"_"+case_name_string+"_lat-"+str(latidxs[0])+"-"+str(latidxs[1])+
                    "_lon-"+str(lonidxs[0])+"-"+str(lonidxs[1]) , bbox_inches='tight', pad_inches=0.1)
    else:
        if latidxs==None or lonidxs==None:
            fig.savefig("./plots/time_series_spinup_"+var_name+"_"+lvl+"_"+case_name_string , bbox_inches='tight', pad_inches=0.1)
        else:
            fig.savefig("./plots/time_series_spinup_"+var_name+"_"+lvl+"_"+case_name_string+"_lat-"+str(latidxs[0])+"-"+str(latidxs[1])+
                    "_lon-"+str(lonidxs[0])+"-"+str(lonidxs[1]) , bbox_inches='tight', pad_inches=0.1)
...
def calc_file_all_in_one(file_path,var_name,var_dim,case_name,layer=None,latidxs=None,lonidxs=None):
    day_sum  = 11020
    if var_dim == 3:
        var = np.zeros([day_sum])
    elif var_dim == 4:
...
def calc_diff_all_in_one(file_paths,case_names,var_name,var_dim,layer):
    day_sum  = 11020
    if var_dim == 3:
...
def plot_file_type_all_in_one(var_name,case_names,latidxs=None,lonidxs=None):
    ...
    for i, case_name in enumerate(case_names):
        if latidxs==None or lonidxs==None:
            if scale == -273.15:
                var[i,:] = np.loadtxt("./txt/"+var_name+"_"+case_name+".txt",dtype='float')*1.0 + scale
            else:
            ...
            if scale == -273.15:
                var[i,:] = np.loadtxt("./txt/"+var_name+"_"+case_name+"_lat-"+str(latidxs[0])+"-"+str(latidxs[1])+
                           "_lon-"+str(lonidxs[0])+"-"+str(lonidxs[1])+".txt",dtype='float')*1.0 + scale
            else:
                var[i,:] = np.loadtxt("./txt/"+var_name+"_"+case_name+"_lat-"+str(latidxs[0])+"-"+str(latidxs[1])+
...
def plot_file_type_diff_all_in_one(file_paths,case_names,var_name,var_dim,layer):
...
if __name__ == "__main__":
        ...
        for var_name in var_names:
            if file_type == 'all_in_one':
                for i in np.arange(len(file_paths)):
                    calc_file_all_in_one(file_paths[i],var_name,var_dim,case_names[i],layer=layer,latidxs=latidxs,lonidxs=lonidxs)
    ...
    if plot_type == "diff":
        ...
        for var_name in var_names:
            ...
            plot_file_type_diff_all_in_one(file_paths,case_names,var_name,var_dim,layer)
