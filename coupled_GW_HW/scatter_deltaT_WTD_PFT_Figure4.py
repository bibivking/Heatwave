...
def plot_spatial_land_days(file_paths, time_s, time_e, seconds=None,loc_lat=None, loc_lon=None, message=None):
    ...
    Tair1  = file1.variables['Tair_f_inst']
    tair1  = time_clip_var(time,Tair1,time_s,time_e,seconds)
    tmax1  = time_clip_var_max(time,Tair1,time_s,time_e,seconds)
    tmin1  = time_clip_var_min(time,Tair1,time_s,time_e,seconds)

    Qle1   = file1.variables['Qle_tavg']
    qle1   = time_clip_var(time,Qle1,time_s,time_e,seconds)
    Qh1    = file1.variables['Qh_tavg']
    qh1    = time_clip_var(time,Qh1,time_s,time_e,seconds)
    TVeg1  = file1.variables['TVeg_tavg']
    tveg1  = time_clip_var(time,TVeg1,time_s,time_e,seconds)
    Evap1  = file1.variables['Evap_tavg']
    evap1  = time_clip_var(time,Evap1,time_s,time_e,seconds)
    VegT1  = file1.variables['VegT_tavg']
    vegt1  = time_clip_var(time,VegT1,time_s,time_e,seconds)
    FW1    = file1.variables['FWsoil_tavg']
    fw1    = time_clip_var(time,FW1,time_s,time_e,seconds)

    if len(file_paths) > 1:
        file2  = Dataset(file_paths[1], mode='r')
        Tair2  = file2.variables['Tair_f_inst']

        tair2  = time_clip_var(time,Tair2,time_s,time_e,seconds)
        tmax2  = time_clip_var_max(time,Tair2,time_s,time_e,seconds)
        tmin2  = time_clip_var_min(time,Tair2,time_s,time_e,seconds)

        Qle2   = file2.variables['Qle_tavg']
        qle2   = time_clip_var(time,Qle2,time_s,time_e,seconds)
        Qh2    = file2.variables['Qh_tavg']
        qh2    = time_clip_var(time,Qh2,time_s,time_e,seconds)
        TVeg2  = file2.variables['TVeg_tavg']
        tveg2  = time_clip_var(time,TVeg2,time_s,time_e,seconds)
        Evap2  = file2.variables['Evap_tavg']
        evap2  = time_clip_var(time,Evap2,time_s,time_e,seconds)
        VegT2  = file2.variables['VegT_tavg']
        vegt2  = time_clip_var(time,VegT2,time_s,time_e,seconds)
        FW2    = file2.variables['FWsoil_tavg']
        fw2    = time_clip_var(time,FW2,time_s,time_e,seconds)

        iveg    = file2.variables["Landcover_inst"]
        WTD_tmp = file2.variables["WaterTableD_tavg"]
        wtd     = time_clip_var(time,WTD_tmp,time_s,time_e,seconds)
    ...
    vegt_shrink    = np.reshape(vegt[:,6:-5,6:-5],-1)
    fw_shrink      = np.reshape(fw[:,6:-5,6:-5],-1)
    print("np.any(np.isnan(wtd_shrink))",np.any([wtd_shrink == wtd_shrink]))
    # print("wtd_shrink",~ np.isnan(wtd_shrink))
    
    iveg_1D    = iveg_shrink[wtd_shrink == 0.]
    wtd_1D     = wtd_shrink[wtd_shrink == 0.]
    tair_1D    = tair_shrink[wtd_shrink == 0.]
    tmax_1D    = tmax_shrink[wtd_shrink == 0.]
    tmin_1D    = tmin_shrink[wtd_shrink == 0.]
    qle_1D     = qle_shrink[wtd_shrink == 0.]
    qh_1D      = qh_shrink[wtd_shrink == 0.]
    tveg_1D    = tveg_shrink[wtd_shrink == 0.]
    evap_1D    = evap_shrink[wtd_shrink == 0.]
    vegt_1D    = vegt_shrink[wtd_shrink == 0.]
    fw_1D      = fw_shrink[wtd_shrink == 0.]
    ...
    fig.savefig("./plots/figures/scatter_delta_Tmax_WTD_PFT_days_"+message,bbox_inches='tight')#, pad_inches=0.1)
...
if __name__ == "__main__":
    ...
    for case_name in case_names:
        ...
        if case_name == "hw2009_3Nov":
            ...
            time_e = datetime(2009,1,31,23,59,0,0)
        ...
        plot_spatial_land_days(file_paths, time_s, time_e, seconds=seconds, message=message)
