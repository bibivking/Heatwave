...
def get_time_cood(file_path, time_s ,time_e):
    ...
    # time_tmp     = time[time_cood_all]
    # doy_all      = [ time_tmp[i].days for i in np.arange(len(time_tmp)) ]
    #
    # time_tmp     = time[time_cood_day]
    # doy_day      = [ time_tmp[i].days for i in np.arange(len(time_tmp)) ]
    #
    # time_tmp     = time[time_cood_night]
    # doy_night    = [ time_tmp[i].days for i in np.arange(len(time_tmp)) ]

    return time_cood_all, time_cood_day, time_cood_night
...
def get_vertcross(file_path, z, t, s, ua, wa, lat_slt, lon_min, lon_max):
    ...
    t_cross    = vertcross(t, z, wrfin=ncfile, start_point=start_point,
                           end_point=end_point, latlon=True, meta=True)
    s_cross    = vertcross(s, z, wrfin=ncfile, start_point=start_point,
                           end_point=end_point, latlon=True, meta=True)
    ua_cross   = vertcross(ua, z, wrfin=ncfile, start_point=start_point,
                           end_point=end_point, latlon=True, meta=True)
    wa_cross   = vertcross(wa, z, wrfin=ncfile, start_point=start_point,
                           end_point=end_point, latlon=True, meta=True)

    return t_cross, s_cross, ua_cross, wa_cross


def get_interpolation(t_crs, s_crs, ua_crs, wa_crs, xy_loc, vertical):

    grid_X, grid_Y = np.meshgrid(xy_loc,vertical)
    vertical_tmp   = to_np(t_crs.coords['vertical'])[:]
    print("vertical_tmp",vertical_tmp)
...
def plot_profile_wrf_wind(file_paths, time_s, time_e, message=None, lat_slt=36, lon_min=130, lon_max=160):
    ...
    time_cood_all, time_cood_day, time_cood_night = get_time_cood(file_paths[0], time_s, time_e)
    ...
    z1_day, t1_day, s1_day, ua1_day, wa1_day = \
                       get_average(Z1,T1,S1,Ua1,Wa1,time_cood_day)
    print("z1_day",z1_day)

    z1_night, t1_night, s1_night, ua1_night, wa1_night = \
                       get_average(Z1,T1,S1,Ua1,Wa1,time_cood_night)

    # ****************** calc vertcross ******************
    t1_day_crs, s1_day_crs, ua1_day_crs, wa1_day_crs =\
        get_vertcross(file_paths[0], z1_day, t1_day, s1_day, ua1_day,
                      wa1_day, lat_slt, lon_min, lon_max)
    print("t1_day_crs", t1_day_crs)

    t1_night_crs, s1_night_crs, ua1_night_crs, wa1_night_crs =\
        get_vertcross(file_paths[0], z1_night, t1_night, s1_night, ua1_night, wa1_night, lat_slt, lon_min, lon_max)

    # ****************** calc interpolation ******************
    xy_loc   = np.linspace(lon_min, lon_max, len(t1_day_crs.coords['xy_loc']))
    vertical = np.arange(0, 5100., 100.)

    t1_day_out, s1_day_out, ua1_day_out, wa1_day_out = \
        get_interpolation(t1_day_crs, s1_day_crs, ua1_day_crs,
                          wa1_day_crs, xy_loc, vertical)
    print("t1_day_out", t1_day_out)

    t1_night_out, s1_night_out, ua1_night_out, wa1_night_out = \
        get_interpolation(t1_night_crs, s1_night_crs,
                          ua1_night_crs, wa1_night_crs, xy_loc, vertical)
    ...
    if len(file_paths) > 1:
        ...
        z2_day, t2_day, s2_day, ua2_day, wa2_day = \
                           get_average(Z2,T2,S2,Ua2,Wa2,time_cood_day)

        z2_night, t2_night, s2_night, ua2_night, wa2_night = \
                           get_average(Z2,T2,S2,Ua2,Wa2,time_cood_night)

        t2_day_crs, s2_day_crs, ua2_day_crs, wa2_day_crs =\
            get_vertcross(file_paths[1], z2_day, t2_day, s2_day, ua2_day, wa2_day, lat_slt, lon_min, lon_max)

        t2_night_crs, s2_night_crs, ua2_night_crs, wa2_night_crs =\
            get_vertcross(file_paths[1], z2_night, t2_night, s2_night, ua2_night, wa2_night, lat_slt, lon_min, lon_max)

        t2_day_out, s2_day_out, ua2_day_out, wa2_day_out = \
            get_interpolation(t2_day_crs, s2_day_crs,
                              ua2_day_crs, wa2_day_crs, xy_loc, vertical)

        t2_night_out, s2_night_out, ua2_night_out, wa2_night_out = \
            get_interpolation(t2_night_crs, s2_night_crs,
                              ua2_night_crs, wa2_night_crs, xy_loc, vertical)

        # ************** calc difference **************
        t_day_out  = t2_day_out  - t1_day_out
        s_day_out  = s2_day_out  - s1_day_out
        ua_day_out = ua2_day_out - ua1_day_out
        wa_day_out = wa2_day_out - wa1_day_out

        t_night_out  = t2_night_out  - t1_night_out
        s_night_out  = s2_night_out  - s1_night_out
        ua_night_out = ua2_night_out - ua1_night_out
        wa_night_out = wa2_night_out - wa1_night_out
        print("t_day_out", t_day_out)

    else:

        t_day_out  = t1_day_out
        s_day_out  = s1_day_out
        ua_day_out = ua1_day_out
        wa_day_out = wa1_day_out

        t_night_out  = t1_night_out
        s_night_out  = s1_night_out
        ua_night_out = ua1_night_out
        wa_night_out = wa1_night_out
    ...
    # Day temperature
    contour   = ax[0,0].contourf(xy_loc, vertical, t_day_out, levels=levels, cmap=get_cmap("coolwarm"))
    ...
    q         = ax[0,0].quiver(xy_loc[::3], vertical[::3], ua_day_out[::3,::3],
                              wa_day_out[::3,::3], angles='xy', scale_units='xy',
                              scale=scale, pivot='middle')
    ax[0,0].quiverkey(q, X=0.90, Y=1.05, U=scale, label=str(scale)+' m/s', labelpos='E')
    ax[0,0].text(0.02, 0.95, "(a) ΔT_day", transform=ax[0,0].transAxes, fontsize=14, verticalalignment='top', bbox=props)
    ax[0,0].set_xlabel("Longitude", fontsize=12)
    ax[0,0].set_ylabel("Geopotential Height (m)", fontsize=12)

    # Night temperature
    contour   = ax[0,1].contourf(xy_loc, vertical, t_night_out, levels=levels, cmap=get_cmap("coolwarm"))
    cb_var    = fig.colorbar(contour, ax=ax[0,1])
    cb_var.set_label('ΔT ($\mathregular{^o}$C)', loc='center') # rotation=270,
    cb_var.ax[0,1].tick_params(labelsize=12)
    q         = ax[0,1].quiver(xy_loc[::3], vertical[::3], ua_night_out[::3,::3],
                              wa_night_out[::3,::3], angles='xy', scale_units='xy',
                              scale=scale, pivot='middle')
    ax[0,1].quiverkey(q, X=0.90, Y=1.05, U=scale, label=str(scale)+' m/s', labelpos='E')
    ax[0,1].text(0.02, 0.95, "(b) ΔT_night", transform=ax[0,1].transAxes, fontsize=14, verticalalignment='top', bbox=props)
    ax[0,1].set_xlabel("Longitude", fontsize=12)
    ax[0,1].set_ylabel("Geopotential Height (m)", fontsize=12)

    # Day specific humidity
    contour   = ax[1,0].contourf(xy_loc, vertical, s_day_out, levels=levels, cmap=get_cmap("coolwarm"))
    ...
    q         = ax[1,0].quiver(xy_loc[::3], vertical[::3], ua_day_out[::3,::3],
                              wa_day_out[::3,::3], angles='xy', scale_units='xy',
                              scale=scale, pivot='middle')
    ax[1,0].quiverkey(q, X=0.90, Y=1.05, U=scale, label=str(scale)+' m/s', labelpos='E')
    ax[1,0].text(0.02, 0.95, "(c) ΔS_day", transform=ax[1,0].transAxes, fontsize=14, verticalalignment='top', bbox=props)
    ax[1,0].set_xlabel("Longitude", fontsize=12)
    ax[1,0].set_ylabel("Geopotential Height (m)", fontsize=12)


    # Day specific humidity
    contour   = ax[1,1].contourf(xy_loc, vertical, s_night_out, levels=levels, cmap=get_cmap("coolwarm"))
    cb_var    = fig.colorbar(contour, ax=ax[1,1])
    cb_var.set_label('ΔS (g kg$\mathregular{^-1}$)', loc='center') # rotation=270,
    cb_var.ax[1,1].tick_params(labelsize=12)
    q         = ax[1,1].quiver(xy_loc[::3], vertical[::3], ua_night_out[::3,::3],
                              wa_night_out[::3,::3], angles='xy', scale_units='xy',
                              scale=scale, pivot='middle')
    ax[1,1].quiverkey(q, X=0.90, Y=1.05, U=scale, label=str(scale)+' m/s', labelpos='E')
    ax[1,1].text(0.02, 0.95, "(d) ΔS_night", transform=ax[1,1].transAxes, fontsize=14, verticalalignment='top', bbox=props)
