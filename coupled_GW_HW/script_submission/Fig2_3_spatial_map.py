...
def plot_spatial_fwsoil_qle_tmax(case_names, wrf_path, loc_lat=None, loc_lon=None, seconds=None, message=None):
    ...
    label_x = ["Δ$β$","ΔQ$\mathregular{_e}$","ΔT$\mathregular{_{max}}$"]
    ...
    for i, case_name in enumerate(case_names):
        ...
        plot3    = ax[i,2].contourf(lon, lat, tmax, levels=clevs3, transform=ccrs.PlateCarree(),cmap=blue2white,extend='both') #
        ...
        if i == 2:
            ...
            # left - Fwsoil
            cbar = plt.colorbar(plot1, ax=ax[:,0], ticklocation="right", pad=0.1, orientation="horizontal",
            ...
            # middle - Qle
            cbar = plt.colorbar(plot2, ax=ax[:,1], ticklocation="right", pad=0.1, orientation="horizontal",
                                aspect=20, shrink=0.6)
            color_label= "W m$\mathregular{^{-2}}$"
            cbar.set_label(color_label, loc='center',size=12)
            cbar.ax.tick_params(labelsize=10, rotation=45)
            ...
            # right - Tmax
            cbar = plt.colorbar(plot3, ax=ax[:,2], ticklocation="right", pad=0.1, orientation="horizontal",
                                aspect=20, shrink=0.6)
            color_label= "$\mathregular{^o}$C"
            cbar.set_label(color_label, loc='center',size=12)
            cbar.ax.tick_params(labelsize=10, rotation=45)
    ...
    plt.savefig('./plots/Fig1_spatial_map_fwsoil_Qle_Tmax_2009_2013_2019.png',dpi=300)
...
def plot_spatial_qh_qair(case_names, wrf_path, loc_lat=None, loc_lon=None, seconds=None, message=None):
    ...
    fig, ax = plt.subplots(nrows=3, ncols=2, figsize=[9,12],sharex=True, sharey=True, squeeze=True,
                           subplot_kw={'projection': ccrs.PlateCarree()})
    plt.subplots_adjust(wspace=-0.42, hspace=0) # left=0.15,right=0.95,top=0.85,bottom=0.05,
    ...
    # cartopy.feature.ocean is a simple version of more complex NaturalEarthFeature function that is safer to use
    # ocean = NaturalEarthFeature('cultural', 'ocean', scale='50m', 
    #                             edgecolor='none', facecolor="lightgray")
    ...
    for i, case_name in enumerate(case_names):
        ...
        # left - Qh
        clevs1  = [ -140, -120, -100, -80, -60, -40, -20, -10]
    ...
    # plt.tight_layout(pad=0.01)
    plt.savefig('./plots/FigS2_spatial_map_Qh_Qair_2009_2013_2019.png',dpi=300)
...
def plot_spatial_fwsoil_qle_qh_tmax_qair(case_names, wrf_path, loc_lat=None, loc_lon=None, seconds=None, message=None):
    ...
    for i, case_name in enumerate(case_names):
        ...
        if i == 2:
            ...
            # middle right - Tmax
            cbar = plt.colorbar(plot4, ax=ax[:,3], ticklocation="right", pad=0.1, orientation="horizontal",
                                aspect=20, shrink=0.49)
            color_label= "$\mathregular{^o}$C"
            cbar.set_label(color_label, loc='center',size=14)
            cbar.ax.tick_params(labelsize=10, rotation=45)
            ...
            # right - q
            cbar = plt.colorbar(plot5, ax=ax[:,4], ticklocation="right", pad=0.1, orientation="horizontal",
                                aspect=20, shrink=0.49)
            color_label= "g kg$\mathregular{^{-1}}$"
            cbar.set_label(color_label, loc='center',size=14)
            cbar.ax.tick_params(labelsize=10, rotation=45)
...
def plot_spatial_Rnet_LW_SW(case_names, loc_lat=None, loc_lon=None, seconds=None, message=None):
    ...
    for i, case_name in enumerate(case_names):
        ...
        if i == 2 and j == 4:
            ...
            cbar = plt.colorbar(plot1, ax=ax, ticklocation="right", pad=0.1, orientation="horizontal",
        ...
        # cb.ax[i,j].tick_params(labelsize=10)
    # plt.tight_layout(pad=0.01)
    plt.savefig('./plots/FigS3_spatial_map_plot_spatial_Rnet_LW_SW_2009_2013_2019.png',dpi=300)
...
if __name__ == "__main__":
    ...
    #     wrf_path   = "/g/data/w97/mm3972/model/wrf/NUWRF/LISWRF_configs/"+case_name+"/ensemble_avg/wrfout_"+period+"_gw"
    #     cpl_land_file     = '/g/data/w97/mm3972/model/wrf/NUWRF/LISWRF_configs/'+case_name+'/ensemble_avg'
