...
def plot_file_type_all_in_one(var_name,var_dim,io_opt,layer=None):
    if io_opt == 'read':
        day_sum  = 11020
        if var_dim == 3:
            var = np.zeros([day_sum])
        ...
        var.dtype
        np.savetxt("./txt/"+var_name+".txt",var)
    if io_opt == 'plot':
        var = np.loadtxt("./txt/"+var_name+".txt",dtype='float')
        print(var.dtype)
...
if __name__ == "__main__":
    ...
    for var_name in var_names:
        ...
        if file_type == 'monthly':
            ...
            io_opt   = 'plot' # "read"
            plot_file_type_all_in_one(var_name,var_dim,io_opt,layer)
